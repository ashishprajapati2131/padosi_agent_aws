...
class HomePageTests(TestCase):
    ...
    def test_home_page_renders_cms_sections(self):
        ...
        SiteSetting.set_value('homepage_content', {
            'dyk': {'visible': True, 'slides': [{'accent': 'accent-rose', 'bg': 'bg-rose-500', 'icon': 'users', 'title': '3× faster claim settlements', 'body': 'Nearby agents help claims clear faster.'}]},
            'quickpicks': {'visible': True, 'items': [{'label': 'Mediclaim', 'badge': 'Most Bought', 'icon': 'heart-pulse', 'url': '/find-agents/'}]},
            'why_choose': {'visible': True, 'cards': [{'stat': '0', 'caption': 'Spam Calls', 'icon': 'shield-check', 'title': 'Privacy-first by design', 'body': 'Only you can contact an agent.'}]},
            'works': {'visible': True, 'steps': [{'icon': 'search', 'accent': 'accent-primary', 'badge': '1', 'title': 'Search', 'desc': 'Find verified agents'}]},
        }, 'homepage')
