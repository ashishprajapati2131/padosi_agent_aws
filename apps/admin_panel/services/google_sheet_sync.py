...
def sync_invoice_to_sheet(invoice_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"[GOOGLE SHEET SYNC] Success: {invoice.get('invoice_number')}")
            return True
        else:
            logger.error(f"[GOOGLE SHEET SYNC] Failed: {invoice.get('invoice_number')} Status: {response.status_code} Body: {response.text}")
            return False
