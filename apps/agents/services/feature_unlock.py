...
def get_plan_slugs():
    ...
    return list(SubscriptionPlan.objects.values_list('slug', flat=True))
...
def _rule_applies(rule, plan_slug):
    ...
    if not feature or feature not in FEATURE_ATTR_MAP:
        return False
    plans = rule.get('plans') or list(get_plan_slugs())
...
def sanitize_unlock_rules(raw_rules):
    ...
    for i, rule in enumerate(raw_rules):
        ...
        if not conditions:
            continue
        plans = [p for p in (rule.get('plans') or []) if p in get_plan_slugs()]
        if not plans:
            plans = list(get_plan_slugs())
...
def _rule_plans(rule):
    plans = [p for p in (rule.get('plans') or []) if p in get_plan_slugs()]
    return plans or list(get_plan_slugs())
...
def upsert_plan_unlock_rule(rules, plan_slug, feature, conditions, match='all'):
    ...
    """
    slug = normalize_plan_slug(plan_slug)
    if slug not in get_plan_slugs() or feature not in FEATURE_ATTR_MAP:
